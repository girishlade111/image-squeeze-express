...
import { ProcessingSettings } from '@/lib/image-processing';
...
const SocialPresetsGrid = ({ setSettings }: SocialPresetsGridProps) => {
      ...
      <div className="mx-auto grid max-w-3xl grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        {platforms.map((p) => (
          <Button
            key={p.name}
            variant="outline"
            className="glass-card flex h-auto flex-col gap-1 rounded-2xl border-border/50 px-4 py-5 hover:border-violet/60 hover:shadow-[0_0_15px_hsl(var(--violet)/0.15)]"
